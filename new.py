...
def create_updrs_trend_chart_simple(df):
    """Grafico UPDRS semplificato per paziente - CON LINEE ORIZZONTALI GRIGIE"""
    fig = go.Figure()

    # Aggiungi linee orizzontali grigie ad ogni livello UPDRS
    updrs_values = df['motor_updrs'].unique()
    for updrs_val in updrs_values:
        fig.add_hline(
            y=updrs_val, 
            line_dash="dot", 
            line_color="#CCCCCC", 
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>Data</b>: %{x|%d/%m/%Y %H:%M}<br><b>UPDRS</b>: %{y:.1f}<extra></extra>'
    ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            showgrid=True, 
            gridcolor='#E0E0E0',
            tickformat='%d/%m/%Y<br>%H:%M'  # Mostra data E ora
        ),
...
def create_updrs_trend_chart_medico(df):
    ...
    fig.update_layout(
        ...
        hovermode='x unified',
        height=400,
        xaxis=dict(tickformat='%d/%m/%Y<br>%H:%M')  # Data E ora anche per medico
    )
                    ...
                    st.markdown(
                        ...
                        ">
                            <p style="color: #333; font-weight: bold; margin-bottom: 10px;">
                                📅 {row['timestamp'].strftime('%d/%m/%Y %H:%M')} - UPDRS: {row['motor_updrs']:.1f}
                            </p>
